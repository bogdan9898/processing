...
void gravitatie() {
  if (piece.inBounds(piece.type, 0) && piece.inTouch() == false) {
    for (int i = 0; i < 4; i++) {
      piece.c[i].y += size;
    }
  } else {
...
void keyPressed() {
  ...
  case DOWN-ARROW:
    println();
    if (piece != null && piece.inBounds(piece.type, 0))
      if (piece.inTouch() == false)
        piece.accel();
    break;
  case RIGHT-ARROW:
    if (piece != null && piece.inBounds(piece.type, 1))
