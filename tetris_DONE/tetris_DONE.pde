...
void draw() {
  ...
  if (!pause) {
    ...
    if (piece != null) {
      if (frameCount % int(difficulty) == 0) {
        gravitatie();
        //difficulty -=0.001;
      }
      piece.update();
...
void gravitatie() {
  if (piece.inBounds(0) && piece.inTouch() == false) {
    ...
    piece = nextPiece;
    nextPiece = new PIECE((width-offsetWidth)/2-size, 0, floor(random(7)));
    if (table[int(piece.c[0].x/size)][int(piece.c[0].y/size)].isFree == false||
      table[int(piece.c[1].x/size)][int(piece.c[1].y/size)].isFree == false||
      table[int(piece.c[2].x/size)][int(piece.c[2].y/size)].isFree == false||
      table[int(piece.c[3].x/size)][int(piece.c[3].y/size)].isFree == false) {
      println("Game Over");
      noLoop();
    }
  }
